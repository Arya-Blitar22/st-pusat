...
source <(curl -sSL 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/module/module')
...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
  ...
  rm -rf /usr/bin/udp
  source <(curl -sSL 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/module/module') &>/dev/null
  wget -O /etc/UDPCustom/module 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/module/module' &>/dev/null
  wget -O /etc/UDPCustom/udp-custom 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/bin/udp-custom' &>/dev/null
  chmod +x /etc/UDPCustom/udp-custom
  chmod +x /etc/UDPCustom/module
  bash /etc/UDPCustom/udp-custom 
  wget -O /etc/limiter.sh 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/module/limiter.sh'
  chmod +x /etc/limiter.sh
  cp /etc/limiter.sh /etc/UDPCustom
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/module/udp' 
  chmod +x /usr/bin/udp
  ...
  sleep 3
  title "${a102:-INSTALLATION COMPLETED SAYANK}"
  msg -bar
  time_reboot 10
fi
